...
void PlaylistComponent::saveLibrary()
{
    ...
    ofstream myFile("playlistData.txt");
    for (File& f : trackFiles)
    {
        myFile << f.getFullPathName().toStdString() << endl;
    }
    myFile.close();
}
...
void PlaylistComponent::loadLibrary()
{
    ...
    ifstream myFile("playlistData.txt");
    string filePath;
    if (myFile.is_open())
    {
...
Component* PlaylistComponent::refreshComponentForCell (int rowNumber, int columnId, bool isRowSelected, Component* existingComponentToUpdate)
{
    ...
    if (columnId == 3 || columnId == 4)
    {
        ...
        btn->onClick = [this, rowNumber, columnId]()
        {
            ...
            if (columnId == 3)
            {
                deckGUI1->loadFile(audioURL);
            }
            else if (columnId == 4)
            {
                deckGUI2->loadFile(audioURL);
            }
        };
